...
1.  Visualize the distribution of `score`.
    ...
    ***Yes, it is left-skewed.***
    What does that tell you about how students rate courses?
    ***It tells you most students rate high, but there are some very low ratings.***
    Is this what you expected to see?
    ***Yes***
    Why, or why not?
    ***Because if the course is still taught and the teacher not fired, there should be something good about them.*** 
...
evals %>% 
  summarize(means = mean(score), median = median(score), 
            min = min(score), max = max(score), sd = sd(score),
...
***There is a slight positive relationship between beauty average rating and average evaluation score.***
...
***The jitter adds some interpherance to the points, so that when there is a lot of overlap for points, it doesn't just look like one point, but is a clump of points. The initial scatterplot (especially without a lower alpha level) made it seem like there were a smaller numebr of poitns.***
...
score_bty_fit %>% tidy() %>% flextable() %>% autofit()
```
...
***For a average beauty rating increase of 1, the average evaluation rating increases by 0.07.***
...
***For an average beauty rating of 0, the average evaluation rating is 3.88. This doesn't make sense in the data, since no beauty ratings are 0.***
...
score_gender_fit %>% flextable() %>% autofit()
```

***The linear model is:***
***score = 4.09 + 0.14 * Male***
***A male professor, on average, score 0.14 higher on evaluations than a female professor. On average, a female professor scores 4.09 on the ratings.***
...
score_rank_fit %>% flextable() %>% autofit()
```

***The model is:***
$$\widehat{score}_i = 4.28 - 0.12\times Tenure track_i - 0.14\times Tenured-i$$

***The average rating for a teaching track professor is 4.28. The average rating for a tenure track professor is 0.12 poitns lower than teaching track professor. The average rating for a tenured professor is 0.14 points lower than a teaching professor.***
...
***The model is:***
$$\widehat{score}_i = 4.14 + 0.14\times Teaching_i + 0.02\times Tenure track-i$$
